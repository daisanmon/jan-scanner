...
- 最低利益率と最低見込み利益による仕入れ基準の端末別設定（参考価格と30日平均成約価格の低い方を売価基準に使用）
...
カメラの読み取り対象はEAN-13です。EAN-8は誤検出防止のため手入力で登録できます。QRコードには対応していません。
...
履歴はブラウザの`localStorage`に保存されます。保存キーは`jan-pocket:scan-history`で、スキーマバージョンと履歴の配列を持つJSON形式です。各履歴にはID、JANコード、ISO形式の読み取り日時、登録方法（`camera`または`manual`）、同一JANのスキャン回数が含まれ、照会完了後は商品、全サイズの価格・販売統計、仕入れ評価、その取得時刻も加わります。従来のスキーマv1／v2履歴は自動的に読み込み、新規保存時にv3へ移行します。
